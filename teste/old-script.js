...
function alternateItems(){
    ...
    const lastClassItems = [getLastClass(items[0].classList), getLastClass(items[1].classList), getLastClass(items[2].classList)];
    
    for(i = 0; i < 3; i++){
        items[i].classList.remove(lastClassItems[i])
    }
    
    const newClasses = alternate(lastClassItems);
